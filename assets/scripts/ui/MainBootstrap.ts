...
const WORD_BANK = [
  ...
  'NOTEBOOK', 'STARFISH', 'SNOWBALL', 'FIREFLY', 'DAYDREAM', 'LIFELONG', 'WATERMELON', 'BLACKBOARD',
...
export class MainBootstrap extends Component {
  ...
  private prepareLevel(): void {
    const spec = this.createLevelSpec(this.levelIndex + 1);
    this.boardSize = spec.boardSize;
    this.levelWords = spec.words;
    this.levelDifficulty = spec.difficulty;
  }

  private createLevelSpec(level: number): LevelSpec {
    const normalized = this.clamp(level, 1, MAX_LEVELS);
    const boardSize = this.boardSizeFor(normalized);
    const wordCount = this.wordCountFor(normalized);
    const minLength = this.minWordLengthFor(normalized);
    const maxLength = this.maxWordLengthFor(normalized, boardSize, minLength);
    return {
      boardSize,
      words: this.pickLevelWords(normalized, wordCount, minLength, maxLength),
      difficulty: this.difficultyFor(normalized),
    };
  }

  private boardSizeFor(level: number): number {
    const rank = this.difficultyRankFor(level);
    if (rank < 5) return rank + 5;
    const phase = this.wavePhaseFor(level);
    return level >= 360 && (phase === 4 || phase === 10) ? 11 : 10;
  }

  private wordCountFor(level: number): number {
    const rank = this.difficultyRankFor(level);
    const progressBonus = Math.floor((level - 1) / 260);
    const peakBonus = this.isPeakLevel(level) ? 1 : 0;
    const baseByRank = [0, 4, 5, 6, 7, 8][rank] ?? 8;
    return this.clamp(baseByRank + Math.floor(progressBonus / 2) + peakBonus, 4, rank >= 5 ? 11 : 9);
  }

  private minWordLengthFor(level: number): number {
    const rank = this.difficultyRankFor(level);
    const progressBonus = Math.floor((level - 1) / 360);
    const baseByRank = [0, 3, 4, 4, 5, 6][rank] ?? 6;
    return this.clamp(baseByRank + progressBonus, 3, rank >= 5 ? 8 : 6);
  }

  private maxWordLengthFor(level: number, boardSize: number, minLength: number): number {
    const rank = this.difficultyRankFor(level);
  ...
  private difficultyFor(level: number): string {
    const rank = this.difficultyRankFor(level);
    if (rank === 1) return '简单';
    if (rank === 2) return '中等';
    if (rank === 3) return '难';
    if (rank === 4) return '困难';
    return this.boardSizeFor(level) >= 11 ? '地狱' : '噩梦';
  }

  private isReliefLevel(level: number): boolean {
    return this.difficultyRankFor(level) <= 2;
  }

  private challengePhase(level: number): number {
    return this.wavePhaseFor(level) + 1;
  }

  private wavePhaseFor(level: number): number {
    return (level - 1) % 12;
  }

  private difficultyRankFor(level: number): number {
    const wave = [1, 2, 3, 4, 5, 4, 2, 3, 4, 5, 5, 3];
    const progressBonus = Math.floor((level - 1) / 250);
    const base = wave[this.wavePhaseFor(level)];
    const bonusCap = base <= 2 ? 1 : base === 3 ? 2 : base === 4 ? 1 : 0;
    return this.clamp(base + Math.min(progressBonus, bonusCap), 1, 5);
  }

  private isPeakLevel(level: number): boolean {
    const phase = this.wavePhaseFor(level);
    return phase === 4 || phase === 10;
  }

  private pickLevelWords(level: number, count: number, minLength: number, maxLength: number): string[] {
    const pool = WORD_BANK.filter((word) => word.length >= minLength && word.length <= maxLength);
    const words: string[] = [];
    let cursor = (level * 7) % pool.length;
    let guard = 0;
  ...
  private diagonalTargetForLevel(): number {
    ...
    const rank = this.difficultyRankFor(this.levelIndex + 1);
